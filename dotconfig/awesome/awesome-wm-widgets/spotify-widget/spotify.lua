...
local GET_SPOTIFY_STATUS_CMD = '/usr/local/bin/sp status'
local GET_CURRENT_SONG_CMD = '/usr/local/bin/sp current'
...
local function worker(user_args)
    ...
    spotify_widget:connect_signal("button::press", function(_, _, _, button)
        if (button == 1) then
            awful.spawn("/usr/local/bin/sp play", false)      -- left click
        elseif (button == 4) then
            awful.spawn("/usr/local/bin/sp next", false)  -- scroll up
        elseif (button == 5) then
            awful.spawn("/usr/local/bin/sp prev", false)  -- scroll down
        end
